...
class MockPrisma {
  user = {
    findFirst: async (args?: any) => {
      if (!args || !args.where) return users[0] || null;
      ...
      if (args.include?.tenant) {
        const tenant = tenants.find((t: any) => t.id === match.tenantId);
    ...
    findMany: async (args?: any) => {
      if (!args || !args.where) return users;
  ...
  tenant = {
    findUnique: async (args?: any) => {
      if (!args || !args.where) return tenants[0] || null;
      const { slug, id, customDomain, subdomain } = args.where;
      let match = tenants.find((t: any) => {
        if (slug && t.slug !== slug) return false;
        if (id && t.id !== id) return false;
        if (customDomain && t.customDomain !== customDomain) return false;
        if (subdomain && t.subdomain !== subdomain) return false;
        return true;
      });
      if (!match) return null;
      let result: any = match;
      if (args.include?.plan) {
        const plan = subscriptionPlans.find((p: any) => p.id === match!.planId);
        result = { ...result, plan: plan || null };
      }
      if (args.include?.settings) {
        const settings = tenantSettingsList.find((s: any) => s.tenantId === match!.id);
    ...
    update: async (args: any) => {
      const index = tenants.findIndex((t: any) => t.id === args.where.id);
      if (index !== -1) {
  ...
  subscriptionPlan = {
    findFirst: async (args?: any) => {
      if (!args || !args.where) return subscriptionPlans[0];
      const { isFree, id, name } = args.where;
      let match = subscriptionPlans.find((p: any) => {
        if (isFree !== undefined && p.isFree !== isFree) return false;
    ...
    findUnique: async (args?: any) => {
      ...
      return subscriptionPlans.find((p: any) => (id && p.id === id) || (name && p.name === name)) || null;
    },
    ...
    update: async (args: any) => {
      const index = subscriptionPlans.findIndex((p: any) => p.id === args.where.id);
      if (index !== -1) {
  ...
  tenantSettings = {
    findFirst: async (args?: any) => {
      if (!args || !args.where) return tenantSettingsList[0];
      const { tenantId, id } = args.where;
      return tenantSettingsList.find((s: any) => (tenantId && s.tenantId === tenantId) || (id && s.id === id)) || null;
    },
    findUnique: async (args?: any) => {
    ...
    update: async (args: any) => {
      const index = tenantSettingsList.findIndex((s: any) => (args.where.id && s.id === args.where.id) || (args.where.tenantId && s.tenantId === args.where.tenantId));
      if (index !== -1) {
  ...
  staff = {
    findMany: async (args?: any) => {
      if (!args || !args.where) return staffList;
      const { tenantId, isActive } = args.where;
      let result = staffList.filter((s: any) => {
        if (tenantId && s.tenantId !== tenantId) return false;
        if (isActive !== undefined && s.isActive !== isActive) return false;
        return true;
      });
      if (args.orderBy?.createdAt === 'asc') result.sort((a: any, b: any) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
      return result;
    },
    findUnique: async (args?: any) => {
      if (!args?.where?.id) return null;
      return staffList.find((s: any) => s.id === args.where.id) || null;
    },
    findFirst: async (args?: any) => {
      if (!args?.where) return staffList[0] || null;
      const { id, tenantId } = args.where;
      return staffList.find((s: any) => {
    ...
    count: async (args?: any) => {
      if (!args?.where) return staffList.length;
      const { tenantId, isActive } = args.where;
    ...
    update: async (args: any) => {
      const index = staffList.findIndex((s: any) => s.id === args.where.id);
      if (index !== -1) {
  ...
  service = {
    findMany: async (args?: any) => {
      if (!args || !args.where) return servicesList;
      const { tenantId, isActive } = args.where;
      let result = servicesList.filter((s: any) => {
        if (tenantId && s.tenantId !== tenantId) return false;
        if (isActive !== undefined && s.isActive !== isActive) return false;
        return true;
      });
    ...
    findUnique: async (args?: any) => {
      if (!args?.where?.id) return null;
      return servicesList.find((s: any) => s.id === args.where.id) || null;
    },
    findFirst: async (args?: any) => {
      if (!args?.where) return servicesList[0] || null;
      const { id, tenantId } = args.where;
      return servicesList.find((s: any) => {
        if (id && s.id !== id) return false;
        if (tenantId && s.tenantId !== tenantId) return false;
        return true;
      }) || null;
    ...
    update: async (args: any) => {
      const index = servicesList.findIndex((s: any) => s.id === args.where.id);
      if (index !== -1) {
  ...
  customer = {
    findMany: async (args?: any) => {
      ...
      if (args?.where) {
        ...
        result = result.filter((c: any) => {
          ...
          if (requiresDeposit !== undefined && c.requiresDeposit !== requiresDeposit) return false;
      ...
      if (args?.include?.appointments) {
        result = result.map((c: any) => ({
          ...c,
          appointments: appointmentsList.filter((a: any) => {
            if (a.customerId !== c.id) return false;
            if (args.include.appointments.where?.status && a.status !== args.include.appointments.where.status) return false;
            return true;
          }).map((a: any) => ({
            ...a,
            ...(args.include.appointments.select ? Object.fromEntries(
              Object.keys(args.include.appointments.select).map((k: string) => [k, a[k]])
            ) : {})
          }))
        }));
      }
      if (args?.orderBy?.name === 'asc') result.sort((a: any, b: any) => a.name.localeCompare(b.name));
      if (args?.take) result = result.slice(0, args.take);
      return result;
    },
    findFirst: async (args?: any) => {
      if (!args?.where) return customersList[0] || null;
      const { id, tenantId, phone } = args.where;
      return customersList.find((c: any) => {
    ...
    findUnique: async (args?: any) => {
      if (!args?.where?.id) return null;
      return customersList.find((c: any) => c.id === args.where.id) || null;
    },
    create: async (args: any) => {
      const newCustomer = {
    ...
    update: async (args: any) => {
      const index = customersList.findIndex((c: any) => c.id === args.where.id);
      if (index !== -1) {
  ...
  appointment = {
    findMany: async (args?: any) => {
      ...
      if (args?.where) {
        ...
        result = result.filter((a: any) => {
          ...
          if (dateTime?.gte && new Date(a.dateTime) < new Date(dateTime.gte)) return false;
          if (dateTime?.lte && new Date(a.dateTime) > new Date(dateTime.lte)) return false;
          if (status?.not && a.status === status.not) return false;
          if (status && typeof status === 'string' && a.status !== status) return false;
          return true;
        });
      }
      if (args?.include) {
        result = result.map((a: any) => ({
          ...
          customer: args.include.customer
            ? customersList.find((c: any) => c.id === a.customerId) || null
            : undefined,
          service: args.include.service || args.include.service === true
            ? servicesList.find((s: any) => s.id === a.serviceId) || null
            : undefined,
          staff: args.include.staff || args.include.staff === true
            ? staffList.find((s: any) => s.id === a.staffId) || null
            : undefined
        }));
      }
      if (args?.orderBy?.dateTime === 'asc') result.sort((a: any, b: any) => new Date(a.dateTime).getTime() - new Date(b.dateTime).getTime());
      return result;
    },
    findFirst: async (args?: any) => {
      if (!args?.where) return appointmentsList[0] || null;
      const { id, tenantId, staffId, dateTime, status } = args.where;
      const match = appointmentsList.find((a: any) => {
        if (id && a.id !== id) return false;
        if (tenantId && a.tenantId !== tenantId) return false;
        if (staffId && a.staffId !== staffId) return false;
        if (dateTime?.gte && new Date(a.dateTime) < new Date(dateTime.gte)) return false;
        if (dateTime?.lt && new Date(a.dateTime) >= new Date(dateTime.lt)) return false;
        if (status?.notIn && status.notIn.includes(a.status)) return false;
        return true;
      }) || null;
      if (match && args?.include?.staff) {
        return { ...match, staff: staffList.find((s: any) => s.id === match.staffId) || null };
      }
      return match;
    },
    count: async (args?: any) => {
      if (!args?.where) return appointmentsList.length;
      const { tenantId, dateTime, status } = args.where;
      return appointmentsList.filter((a: any) => {
        if (tenantId && a.tenantId !== tenantId) return false;
        if (dateTime?.gte && new Date(a.dateTime) < new Date(dateTime.gte)) return false;
        if (dateTime?.lt && new Date(a.dateTime) >= new Date(dateTime.lt)) return false;
        if (status?.notIn && status.notIn.includes(a.status)) return false;
    ...
    create: async (args: any) => {
      ...
      const result: any = { ...newApp };
      if (args?.include?.service) result.service = servicesList.find((s: any) => s.id === newApp.serviceId) || null;
      if (args?.include?.staff) result.staff = staffList.find((s: any) => s.id === newApp.staffId) || null;
      return result;
    },
    update: async (args: any) => {
      const index = appointmentsList.findIndex((a: any) => a.id === args.where.id);
      if (index !== -1) {
